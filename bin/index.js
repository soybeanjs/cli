...
var pathStrs = [NODE_MODULE_PATH, "dist", "package-lock.json", "yarn.lock", "pnpm-lock.yaml"];
var deepPathStrs = pathStrs.map((item) => `./**/${item}`);
async function cleanup() {
  const paths = await glob(deepPathStrs, { ignore: "node_modules/**" });
  (0, import_rimraf.rimraf)(paths.concat([NODE_MODULE_PATH]));
}
...
async function initSimpleGitHooks() {
  ...
  const existHusky = (0, import_fs3.existsSync)(huskyDir);
  if (existHusky) {
